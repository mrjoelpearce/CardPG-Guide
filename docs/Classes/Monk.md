The monk rushes into battle, and gets stronger when surrounded by enemies. Just be careful not to overextend or get too far from the rest of your party.

Passive: If you hit the same enemy twice in a turn, inflict Weaken 1.

Starting HP: 6

Recommended Weapons: [Brawling](../Weapons/Brawling.md), [Cudgel](../Weapons/Cudgel.md), [Daggers](../Weapons/Daggers.md), [Greatsword](../Weapons/Greatsword.md), [Rapier](../Weapons/Rapier.md), [Sword & Shield](../Weapons/Sword%20%26%20Shield.md), [Whipblade](../Weapons/Whipblade.md)

### Cards

| Card             | Level | Initiative | Ability                                                                                               |
| ---------------- | ----- | ---------- | ----------------------------------------------------------------------------------------------------- |
| Roundhouse       | 1     | 30         | Attack 1 in a 3 space arc and apply Stun to the targets.                                              |
| Eye of the Storm | 1     | 40         | Attack 2, plus 1 additional damage for each enemy adjacent to you beyond the first.                   |
| Reversal         | 1     | 10         | Ongoing: The next time you are attacked, counterattack for 2 and move 2 spaces. Ends after 1 trigger. |
| Crane Stance     | 1     | 58         | Ongoing: The next three times you are attacked, Retaliate 1. Ends after 3 triggers.                   |
| Deflect          | 1     | 23         | Gain Shield 1 and Retaliate 2.                                                                        |
| Pressure Point   | 2     | 35         | Attack 2 and apply Weaken 2 to the target, then apply Slow to them.                                   |
| Iron Skin        | 2     | 50         | Gain Bolster 2 and Retaliate 3.                                                                       |
| Tempest          | 7     | 25         | Move 2 spaces, then attack 2 against all adjacent enemies.                                            |
| Thousand Cuts    | 7     | 32         | Attack 2 and apply Bleed 3 to the target.                                                             |
| Seven Stars      | 10    | 28         | Expend: Attack 4 and apply Stun to adjacent targets, then push them 3 spaces.                         |
| Empty Mind       | 10    | 65         | Expend: Gain Shield 3 and Boost, then heal 4 HP on yourself.                                          |
### Subclasses

**Martial Artist**: Gain additional mobility options and some harder hitting attacks

| Card               | Level | Initiative | Ability                                                                         |
| ------------------ | ----- | ---------- | ------------------------------------------------------------------------------- |
| Flying Kick        | 4     | 20         | Move 3 spaces, then attack 3 and apply Stun to the target.                      |
| Iron Fist          | 6     | 25         | Attack 4 with Pierce 2.                                                         |
| Ten Thousand Fists | 8     | 28         | Expend: Attack 2, then attack 2, then attack 2 and apply Bleed 3 to the target. |
**Ascendant**: Use your concentration to provide support and sustain in longer fights

| Card        | Level | Initiative | Ability                                                                                |
| ----------- | ----- | ---------- | -------------------------------------------------------------------------------------- |
| Ki Transfer | 4     | 58         | Heal 3 HP on an adjacent ally, then restore 1 card from their discard (their choice).  |
| Transcend   | 6     | 65         | Gain Bless and Boost, then heal 3 HP on yourself and restore 1 card from your discard. |
| Chi Wave    | 8     | 62         | Expend: Heal 4 HP on each ally within 4 spaces, then grant each Bless.                 |
