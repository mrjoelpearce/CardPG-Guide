The Psychic uses mental attacks that bypass enemy defenses, and mind control abilities to coerce enemies to attack each other. Requires thoughtful use of abilities for maximum effect.

Passive: When you kill an enemy, regain 2 health.

Starting HP: 5

Recommended Weapons: [Bow](../Weapons/Bow.md), [Crossbow](../Weapons/Crossbow.md), [Holy Symbol](../Weapons/Holy%20Symbol.md), [Tome](../Weapons/Tome.md), [Wand](../Weapons/Wand.md), [Whipblade](../Weapons/Whipblade.md)

### Cards

| Card              | Level | Initiative | Ability                                                                                                        |
| ----------------- | ----- | ---------- | -------------------------------------------------------------------------------------------------------------- |
| Psychic Lance     | 1     | 35         | Deal 1 direct damage to a target at Range 4, dealing stun.                                                     |
| Mind Shackle      | 1     | 13         | Coerce 1: force all enemies in a 3 space cone to move up to 1 space and attack another combatant for 1 damage. |
| Cloud the Mind    | 1     | 48         | Apply Weaken 1 and Blind to an enemy at Range 3.                                                               |
| Guilty Conscience | 1     | 52         | The next three times you coerce an enemy, that enemy takes 2 damage and gains Curse 1 after making the attack. |
| Mind Spike        | 1     | 60         | At the start of your next three turns, deal one damage to an enemy at range:5                                  |
| Break the Mind    | 2     | 38         | Coerce 3: force an enemy to move up to 3 spaces and attack another combatant for 3 damage.                     |
| Psychic Backlash  | 2     | 55         | Ongoing: The next 3 times you are attacked, deal 2 direct damage back to the attacker.                         |
| Shatter Resolve   | 7     | 35         | Deal 3 direct damage to all enemies within 2 spaces, bypassing their defenses.                                 |
| Phantasmal Image  | 7     | 48         | Deal 3 damage to a target at Range 5, then Hide.                                                               |
| Mind Shatter      | 10    | 25         | Expend: Deal 5 direct damage to a target, bypassing their defenses, then inflict Stun.                         |
| Puppet Master     | 10    | 40         | Expend: Force one enemy to move up to 5 spaces and attack another combatant for 5 damage.                      |

### Subclasses

**Manipulator**: Make better use of your Coerce skills as you direct your enemies.

| Card             | Level | Initiative | Ability                                                                                     |
| ---------------- | ----- | ---------- | ------------------------------------------------------------------------------------------- |
| Twist the Knife  | 4     | 40         | Give an enemy boost, then Coerce 2.                                                         |
| Cruel Master     | 6     | 45         | At the end of your next three turns, inflict an enemy in Range 4 with Shock 2 and Coerce 2. |
| Total Domination | 8     | 30         | Choose two enemies. They use basic attacks against each other until one is dead.            |
**Souleater**: Gain more benefits from dealing direct psychic damage to enemies.

| Card           | Level | Initiative | Ability                                                                                                  |
| -------------- | ----- | ---------- | -------------------------------------------------------------------------------------------------------- |
| Feed on Pain   | 4     | 42         | Deal 2 direct damage to an enemy, then heal 1.                                                           |
| Cleansing Pain | 6     | 50         | Ongoing: Whenever you deal direct damage, Cure a condition. Ends when you discard this card voluntarily. |
| Soul Harvest   | 8     | 25         | Expend: Deal 3 direct damage to all enemies within 3 spaces, then Heal 4.                                |